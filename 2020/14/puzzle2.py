...
def execute(input):
	...
	for line in lines:
		if ('mask' in line):
			...
			splittedLine = line.split('[')
			position = int(splittedLine[1].split(']')[0])
			splittedLine2 = splittedLine[1].split('= ')
			value = int(splittedLine2[1][:-1])
			newPositions = calculateMemoryValue(toBinaryList(position), mask)
			for pos in newPositions:
				memory[toNumber(pos)] = value
...
def calculateMemoryValue(binaryList, mask):
	...
	for i in range(0, len(mask)):
		if (mask[i] == '0'):
			...
			newBinaryList.append('X')
		elif (mask[i] == '1'):
			newBinaryList.append(1)
	listOfAdresses = [newBinaryList]
	while (len([adress for adress in listOfAdresses if 'X' in adress]) != 0):
		counter = 0
		current = listOfAdresses[counter]
		while (not ('X' in current)):
			counter += 1
			current = listOfAdresses[counter]
		listOfAdresses.remove(current)
		listOfAdresses += createOptions(current)

	return listOfAdresses

def createOptions(mask):
	option1 = copy.deepcopy(mask)
	option2 = copy.deepcopy(mask)
	for i in range(0, len(mask)):
		if (mask[i] == 'X'):
			option1[i] = 0
			option2[i] = 1
